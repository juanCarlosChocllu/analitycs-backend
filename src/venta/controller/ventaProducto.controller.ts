...
export class VentaProductoController {
  ...
  reporteProductosActual(@Body() buscadorVentaDto: BuscadorVentaDto) {
    return this.ventaProductoService.reporteVentaProductosUnidad(
      buscadorVentaDto,
      true,
    );
  }
  ...
  detalleProductoKpi(
    ...
    @Param('rubro') rubro: string,
    @Body() detalleVentaDto: DetalleVentaDto,
  ) {
    return this.ventaProductoService.detalleProductoKpi(
      detalleVentaDto,
      sucursal,
      rubro,
    );
